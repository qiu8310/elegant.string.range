...
(function() {
  ...
  /**
   *
   * @param start {number} start index of your content
   * @param end {number} end index of your content
   * @param data {*} [optional] this is for yourself
   * @constructor
   */
  function Range(start, end, data) {
    if (start > end) {
      throw new Error('Range start can not large then end.');
    }
    this.start = start;
    this.end = end;
    this.data = data;
  ...
  RangedContent.prototype = {
     ...
     *
     * @memberOf RangedContent
     * @param start {number}  Range 的起始位置
     * @param end {number} Range 的结束位置
     * @returns {boolean}
     */
    ...
    replace: function(fn) {
      ...
      this.each(function(range, i, ref) {
        ...
        rtn.push(fn.call(range, range.data));
        start = range.end;
      });
